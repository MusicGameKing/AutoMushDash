...
class RealTimeWindowDetection:
    ...
    boxs =[box("hold",0,default_late,default_destion),
           ...
           box("cheboss",4,default_late,default_destion),
    ...
    def simulate_key_press(self,detections):
        ...
        for detection in detections:
            ...
            for box in self.boxs:
                if box.style == class_name:
                    ...
                    if type != -1:
                        if type == 0:
                            if bbox[0] < box.destion - self.lead_range:
                            ...
                            if bbox[0] < box.destion - self.lead_range:
                                self.late_presscode_add("j", box.late, True)
                                self.late_presscode_add("j", box.late+30, False)
                                self.late_presscode_add("f", box.late, True)
                                self.late_presscode_add("f", box.late+30, False)
                                self.box_lasttime_in[index] = True
                                break
                        #print(f"检测到目标: {class_name} \t (置信度: {confidence:.2f}) \t 中心点: {center}")
                        elif abs(center[0] - box.destion) <= self.lead_range:
                            ...
                            if type == 1:
                                if (self.box_lasttime_in[index] == False):
                                    if center[1] > self.upanddown_range:
                                        self.late_presscode_add("j", box.late, True)
                                        # self.late_presscode_add("j", box.late+50, False)
                                    else:
                                        self.late_presscode_add("f", box.late, True)
                                        # self.late_presscode_add("f", box.late+50, False)
                            elif type == 2:
                                if (self.box_lasttime_in[index] == False):
                                    if center[1] <= self.upanddown_range:
                                        self.late_presscode_add("j", box.late, True)
                                ...
                                self.late_presscode_add("j", box.late, True)
                                #self.late_presscode_add("j", box.late+100, False)
                                self.late_presscode_add("f", box.late, True)
                                #self.late_presscode_add("f", box.late+100, False)
                            
                            self.box_lasttime_in[index] = True
                            break
                        if not self.box_lasttime_in[index]:
                            if type != 2:
                                if center[1] > self.upanddown_range:
        ...
        for i in range(len(self.boxs)):
            self.box_lasttime_in[i] = self.box_in[i]
            self.box_in[i] = False
    ...
    def _detection_loop(self):
        ...
        while self.is_running:
            ...
            if processing_time < 0.005:  # 约30FPS
                time.sleep(0.005 - processing_time)
    ...
    def _display_loop(self):
        ...
        while self.is_running:
            ...
            if hasattr(self, 'last_screenshot') and hasattr(self, 'last_detections'):
                ...
                #画一条y = upanddown_range的直线
                cv2.line(visualized_image, (0, self.upanddown_range), (visualized_image.shape[1], self.upanddown_range), (0, 255, 0), 2)
                #画一条 x = default_destion 的直线
                cv2.line(visualized_image, (self.default_destion, 0), (self.default_destion, visualized_image.shape[0]), (255, 0, 0), 2)
                #画2条 x = default_destion+-lead_range 的直线
                cv2.line(visualized_image, (self.default_destion - self.lead_range, 0), (self.default_destion - self.lead_range, visualized_image.shape[0]), (255, 0, 0), 2)
                cv2.line(visualized_image, (self.default_destion + self.lead_range, 0), (self.default_destion + self.lead_range, visualized_image.shape[0]), (255, 0, 0), 2)
                # 显示图像
            ...
            self.refresh_late_presscode(16)
            # 检查退出条件
